...
Self-hosted single-user web app to monitor a fleet of EVE Online characters you personally own, keep their status visible at a glance, and drive multibox operations from one place: **mass fleet invites**, **mass autopilot waypoints**, **planetary-interaction monitoring**, and **manufacturing planning**. Built on CCP's ESI (EVE Swagger Interface).
...
Manufacturing and build-chain planning for answering "what will this blueprint take this pilot to build?" without opening external tools.

- **Pilot picker** — choose **Max skills** (a virtual all-V pilot) or any authenticated character. Real pilots use the cached `read_skills.v1` snapshot; if the scheduler has not polled skills yet, the quote returns a "wait for skill poll" message.
- **Blueprint search** — type at least 2 characters to search manufacturing blueprints parsed from the current SDE-derived data. Results include the blueprint name, product name, and output quantity.
...
- **Build chain planner** — optional "from nothing" view that starts from the selected output and walks invention, blueprint copying, the T1 source hull, recursive manufacturable inputs, and reactions where present in the SDE activity data.
- **Build system** — system search pulls live ESI industry cost indexes for manufacturing, invention, copying, research, and reactions. Private structure rigs are not available from public ESI, so the UI exposes manual structure/rig bonus fields for material, time, and job-fee reductions.
- **Invention modeling** — source blueprint, expected attempts, copy runs, datacores, decryptor effects, invented BPC ME/TE/runs, invention chance, and required science/encryption skills.
- **From-nothing skills** — the planner expands required job skills through skill prerequisites, then estimates SP/time from the selected pilot's cached skills and attributes.
...
- `GET /api/industry/plan?...` — returns the build-chain plan, jobs, invention summary, raw material roll-up, recursive skill plan, live system indexes, and estimated install fees when adjusted prices are available.
- `GET /api/industry/system-costs?systemId=<id>` — returns live ESI cost indexes for a selected solar system.

The build-chain planner is intentionally still an estimator: public ESI does not reveal private Upwell structure rigs, taxes, or exact facility setup, so those bonuses are user inputs. Job fees are estimated from ESI adjusted prices and system cost indexes.

Industry data is generated by the same SDE pipeline as Skills:

```bash
npm run build:mastery
```

That command reads CCP's YAML SDE for mastery certificates, then overlays Fuzzwork's latest CSV dump for current type, dogma, and industry tables. This matters because CCP's old `tranquility/sde.zip` URL may lag behind current hull releases. The generated JSON and downloaded caches stay local/gitignored.
...
- `src/routes/industry.ts` + `src/industry/calculator.ts` + `src/industry/planner.ts` — manufacturing blueprint search (`/api/industry/blueprints`), quick quote calculation (`/api/industry/quote`), system cost lookup (`/api/industry/system-costs`), and build-chain planning (`/api/industry/plan`). Quotes support Max skills or real pilot skill snapshots, runs, ME, TE, SP gaps, and train-time estimates. Plans add invention, copy jobs, recursive input manufacturing/reactions, system cost indexes, estimated install fees, decryptors, and manual structure/rig bonus modeling.
...
| Industry blueprint search is empty or quotes 404                                     | The generated mastery/industry JSON is missing or stale. Run `npm run build:mastery` so `data/eve-mastery.json` includes manufacturing blueprints. |
