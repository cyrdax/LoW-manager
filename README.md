...
Manufacturing-only v1 for answering "what will this blueprint cost this pilot to run?" without opening external tools.
...
- **Blueprint search** — type at least 2 characters to search manufacturing blueprints parsed from CCP's SDE. Results include the blueprint name, product name, and output quantity.
...
That command now parses `fsd/blueprints.yaml` and writes manufacturing blueprint data into `data/eve-mastery.json`. The generated JSON and the downloaded SDE cache stay local/gitignored.
