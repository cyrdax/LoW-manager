...
test('fit price widget opens an itemized category breakdown modal', () => {
  ...
  assert.match(fitsView, /<PriceBreakdownModal quote=\{quote\} fit=\{active\} \/>/);
  assert.match(fitsView, /<Modal title="Fit Price Breakdown" className="fits-price-modal" bodyClassName="fits-price-modal-body"/);
  assert.match(fitsView, /function PriceBreakdownModal\(\{ quote, fit \}: \{ quote: FitQuote; fit: FitDraft \| SavedFitDetail \}\)/);
  assert.match(fitsView, /priceGroupsForFit\(fit, quote\)\.map/);
  ...
  assert.match(styles, /\.fits-price-modal-body/);
  assert.match(styles, /width: min\(1320px, calc\(100vw - 24px\)\);/);
  assert.match(styles, /\.fits-price-breakdown/);
  assert.match(styles, /\.fits-price-modal-body \{[\s\S]*?min-height: 0;[\s\S]*?overflow-y: auto;/);
