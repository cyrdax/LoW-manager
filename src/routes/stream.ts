...
export function registerStreamRoute(app: FastifyInstance, deps: StreamRouteDeps = {}) {
  ...
  app.get('/api/stream', async (req, reply) => {
    ...
    const characterIds = async () => new Set(await characterStore.listIdsByUser(user.id));
    const ownsCharacter = async (value: unknown) => {
      if (typeof value !== 'object' || value === null || !('characterId' in value)) return false;
      return (await characterIds()).has(Number((value as { characterId: unknown }).characterId));
    };

    const ids = await characterIds();
    send('snapshot', snapshot().filter(c => ids.has(c.characterId)));

    const onStatus = (update: unknown) => {
      void ownsCharacter(update).then(ok => { if (ok) send('status', update); });
    };
    const onRemoved = (update: unknown) => {
      void ownsCharacter(update).then(ok => { if (ok) send('removed', update); });
    };
